use crate::{
    data_types::{
        ...
        page_info::{PageInfo, Phrase, Tense},
    },
    helper_functions::{
        ...
        save_functions::{
            save_data_to_file, save_env, save_json_data_vec_to_file, save_map_vec_to_file,
            save_string_vec_vec_to_file,
        },
    },
};

use std::{collections::BTreeMap, env, fs::read_to_string, thread, time::Duration};
...
pub async fn run_conjugations_modules() {
    ...
    let (_language_data_vec, language_vec) =
        read_language_data_from_json_data(language_content.as_str());
    ...
    let spanish_verb_page_info_vec_vec_content: String =
        read_file_to_string("temp/json/conjugations/data/Spanish_page_info.json");
    let spanish_verb_page_info_vec_vec: Vec<PageInfo> =
        serde_json::from_str(&spanish_verb_page_info_vec_vec_content).unwrap();
    let portuguese_verb_page_info_vec_vec_content: String =
        read_file_to_string("temp/json/conjugations/data/Portuguese_page_info.json");
    let portuguese_verb_page_info_vec_vec: Vec<PageInfo> =
        serde_json::from_str(&portuguese_verb_page_info_vec_vec_content).unwrap();
    let italian_verb_page_info_vec_vec_content: String =
        read_file_to_string("temp/json/conjugations/data/Italian_page_info.json");
    let italian_verb_page_info_vec_vec: Vec<PageInfo> =
        serde_json::from_str(&italian_verb_page_info_vec_vec_content).unwrap();
    let french_verb_page_info_vec_vec_content: String =
        read_file_to_string("temp/json/conjugations/data/French_page_info.json");
    let french_verb_page_info_vec_vec: Vec<PageInfo> =
        serde_json::from_str(&french_verb_page_info_vec_vec_content).unwrap();
    let english_verb_page_info_vec_vec_content: String =
        read_file_to_string("temp/json/conjugations/data/English_page_info.json");
    let english_verb_page_info_vec_vec: Vec<PageInfo> =
        serde_json::from_str(&english_verb_page_info_vec_vec_content).unwrap();
    ...
    let language_pk_content: String =
        read_file_to_string("temp/json/languages/btreemaps/language_pk.json");
    let language_pk_map: BTreeMap<String, i64> =
        serde_json::from_str(&language_pk_content).unwrap();
    let model_content: String =
        read_file_to_string("temp/json/models/btreemaps/model_language_id.json");
    let model_language_id_map_vec: Vec<BTreeMap<String, i64>> =
        serde_json::from_str(&model_content).unwrap();

    // Fetch data vectors and then create and save json data vectors
    let base_data_vec_vec: Vec<Vec<String>> =
        extract_base_data_vec_vec(&verb_page_info_vec_vec, &language_pk_map);
    let base_json_data_vec: Vec<JsonData> =
        create_json_data_vec_from_vec_vec_string(&base_data_vec_vec, FieldOptions::BaseField);
    save_json_data_vec_to_file(&base_json_data_vec, "temp/json/conjugations/bases.json");
    let base_pk_map_vec: Vec<BTreeMap<String, i64>> =
        get_base_pk_map_vec(base_json_data_vec.clone(), &language_vec);
    save_map_vec_to_file(
        &base_pk_map_vec,
        "temp/json/conjugations/btreemaps/bases_pk.json",
    );

    // CREATE MAJOR AND MINOR TENSE JSONDATA VECS FOR THIS AND LINK THOSE TO TENSE
    let major_tense_data_vec_vec: Vec<Vec<String>> =
        extract_major_tense_data_vec_vec(&verb_page_info_vec_vec, &language_pk_map);
    let major_tense_json_data_vec: Vec<JsonData> = create_json_data_vec_from_vec_vec_string(
        &major_tense_data_vec_vec,
        FieldOptions::MajorTenseField,
    );
    save_json_data_vec_to_file(
        &major_tense_json_data_vec,
        "temp/json/conjugations/major_tenses.json",
    );
    let major_tense_pk_map_vec: Vec<BTreeMap<String, i64>> =
        get_major_tense_pk_map_vec(major_tense_json_data_vec.clone(), &language_vec);
    save_map_vec_to_file(
        &major_tense_pk_map_vec,
        "temp/json/conjugations/btreemaps/major_tense_pk.json",
    );

    let minor_tense_data_vec_vec: Vec<Vec<String>> =
        extract_minor_tense_data_vec_vec(&verb_page_info_vec_vec, &language_pk_map);
    let minor_tense_json_data_vec: Vec<JsonData> = create_json_data_vec_from_vec_vec_string(
        &minor_tense_data_vec_vec,
        FieldOptions::MinorTenseField,
    );
    save_json_data_vec_to_file(
        &minor_tense_json_data_vec,
        "temp/json/conjugations/minor_tenses.json",
    );
    let minor_tense_pk_map_vec: Vec<BTreeMap<String, i64>> =
        get_minor_tense_pk_map_vec(minor_tense_json_data_vec.clone(), &language_vec);
    save_map_vec_to_file(
        &minor_tense_pk_map_vec,
        "temp/json/conjugations/btreemaps/minor_tense_pk.json",
    );

    let tense_data_vec_vec: Vec<Vec<String>> = extract_tense_data_vec_vec(
        &verb_page_info_vec_vec,
        &major_tense_pk_map_vec,
        &minor_tense_pk_map_vec,
    );
    let tense_json_data_vec: Vec<JsonData> = create_json_data_vec_from_vec_vec_string(
        &tense_data_vec_vec.clone(),
        FieldOptions::TenseField,
    );
    save_json_data_vec_to_file(&tense_json_data_vec, "temp/json/conjugations/tenses.json");
    let tense_pk_map_vec: Vec<BTreeMap<String, i64>> = get_tense_pk_map_vec(
        tense_json_data_vec.clone(),
        &language_vec,
        &major_tense_pk_map_vec,
    );
    save_map_vec_to_file(
        &tense_pk_map_vec,
        "temp/json/conjugations/btreemaps/tense_pk.json",
    );

    // CREATE BLANK PARTICLES {""} FOR EACH LANGUAGE
    let particle_data_vec_vec: Vec<Vec<String>> =
        extract_particle_data_vec_vec(&verb_page_info_vec_vec, &language_pk_map);
    let particle_json_data_vec: Vec<JsonData> = create_json_data_vec_from_vec_vec_string(
        &particle_data_vec_vec,
        FieldOptions::ParticleField,
    );
    save_json_data_vec_to_file(
        &particle_json_data_vec,
        "temp/json/conjugations/particles.json",
    );
    let particle_pk_map_vec: Vec<BTreeMap<String, i64>> =
        get_particle_pk_map_vec(particle_json_data_vec.clone(), &language_vec);
    save_map_vec_to_file(
        &particle_pk_map_vec,
        "temp/json/conjugations/btreemaps/particle_pk.json",
    );

    // CREATE BLANK SUBJECTS {""} FOR EACH LANGUAGE
    let subject_data_vec_vec: Vec<Vec<String>> =
        extract_subject_data_vec_vec(&verb_page_info_vec_vec, &language_pk_map);
    let subject_json_data_vec: Vec<JsonData> =
        create_json_data_vec_from_vec_vec_string(&subject_data_vec_vec, FieldOptions::SubjectField);
    save_json_data_vec_to_file(
        &subject_json_data_vec,
        "temp/json/conjugations/subjects.json",
    );
    let subject_pk_map_vec: Vec<BTreeMap<String, i64>> =
        get_subject_pk_map_vec(subject_json_data_vec.clone(), &language_vec);
    save_map_vec_to_file(
        &subject_pk_map_vec,
        "temp/json/conjugations/btreemaps/subject_pk.json",
    );

    // CREATE BLANK AUXILIARIES {""} FOR EACH LANGUAGE
    let auxiliary_data_vec_vec: Vec<Vec<String>> =
        extract_auxiliary_data_vec_vec(&verb_page_info_vec_vec, &language_pk_map);
    let auxiliary_json_data_vec: Vec<JsonData> = create_json_data_vec_from_vec_vec_string(
        &auxiliary_data_vec_vec,
        FieldOptions::AuxiliaryField,
    );
    save_json_data_vec_to_file(
        &auxiliary_json_data_vec,
        "temp/json/conjugations/auxiliaries.json",
    );
    let auxiliary_pk_map_vec: Vec<BTreeMap<String, i64>> =
        get_auxiliary_pk_map_vec(auxiliary_json_data_vec.clone(), &language_vec);
    save_map_vec_to_file(
        &auxiliary_pk_map_vec,
        "temp/json/conjugations/btreemaps/auxiliary_pk.json",
    );
    ...
    let conjugate_data_vec_vec: Vec<Vec<String>> = extract_conjugate_data_vec_vec(
        &verb_page_info_vec_vec,
        &base_pk_map_vec,
        &model_language_id_map_vec,
    );
    let conjugate_json_data_vec: Vec<JsonData> = create_json_data_vec_from_vec_vec_string(
        &conjugate_data_vec_vec,
        FieldOptions::ConjugateField,
    );
    save_json_data_vec_to_file(
        &conjugate_json_data_vec,
        "temp/json/conjugations/conjugates.json",
    );
    let conjugate_pk_map_vec: Vec<BTreeMap<String, i64>> = get_conjugate_pk_map_vec(
        conjugate_json_data_vec.clone(),
        base_json_data_vec.clone(),
        &language_vec,
    );
    save_map_vec_to_file(
        &conjugate_pk_map_vec,
        "temp/json/conjugations/btreemaps/conjugates.json",
    );
    ...
    let conjugation_json_data_vec: Vec<JsonData> = create_json_data_vec_from_vec_vec_string(
        &conjugation_data_vec_vec,
        FieldOptions::ConjugationField,
    );
    save_json_data_vec_to_file(
        &conjugation_json_data_vec,
        "temp/json/conjugations/conjugations.json",
    );
...
async fn fetch_verb_url_vec_vec(
    language_vec: Vec<String>,
    backoff: u64,
    error_429_backoff: u64,
    ...
    let verb_url_vec_vec: Vec<Vec<String>> = match verb_url_vec_vec_file_result {
        ...
        Err(_) => {
            ...
            let (verb_vec_vec, backoff, error_429_backoff) =
                scrape_url_listing_vec_vec(url_listing_vec_vec, backoff, error_429_backoff).await;

            let verb_url_vec_vec: Vec<Vec<String>> =
                generate_verb_url_vec_vec(verb_vec_vec, language_vec);
            save_string_vec_vec_to_file(
                &verb_url_vec_vec,
                "temp/json/conjugations/data/verb_urls.json",
            );
...
fn generate_url_listing_vec_vec(language_vec: &Vec<String>) -> Vec<Vec<String>> {
    ...
    for language in language_vec {
        let url_listing_vec: Vec<String> = url_val_array
            .map(|val| {
...
async fn scrape_url_listing_vec_vec(
    url_listing_vec_vec: Vec<Vec<String>>,
    mut backoff: u64,
    mut error_429_backoff: u64,
) -> (Vec<Vec<String>>, u64, u64) {
    let mut verb_vec_vec: Vec<Vec<String>> = url_listing_vec_vec
        .clone()
        .into_iter()
        .map(|_| Vec::new())
        ...
        for url_listing in url_listing_vec {
            ...
            while valid_response_bool == false {
                ...
                if valid_response_bool == false {
                    ...
                    let error_429_backoff_duration: Duration =
                        Duration::from_secs(error_429_backoff);
            ...
            let mut verb_vec: Vec<String> = section
                .select(&li_selector)
...
fn generate_verb_url_vec_vec(
    verb_vec_vec: Vec<Vec<String>>,
    language_vec: Vec<String>,
) -> Vec<Vec<String>> {
    let verb_url_vec_vec: Vec<Vec<String>> = verb_vec_vec
        .into_iter()
        .enumerate()
        .map(|(index, verb_vec)| {
            verb_vec
                .into_iter()
                .map(|verb| generate_verb_url(&verb, &language_vec[index]))
                .collect::<Vec<String>>()
        })
        .collect::<Vec<Vec<String>>>();
...
async fn generate_verb_page_info_vec_vec(
    language_vec: Vec<String>,
    verb_url_vec_vec: Vec<Vec<String>>,
    mut backoff: u64,
    mut error_429_backoff: u64,
) -> Vec<Vec<PageInfo>> {
    ...
    for (index, verb_url_vec) in verb_url_vec_vec.into_iter().enumerate() {
        let page_info_vec_file_path: String = String::from("temp/json/conjugations/data/")
            + language_vec[index].as_str()
            + "_page_info.json";
        let scraped_verb_vec_file_path: String = String::from("temp/json/conjugations/data/")
            + language_vec[index].as_str()
            + "_scraped_verb_vec.json";

        let page_info_vec_content: String = read_file_to_string_or_none(&page_info_vec_file_path);
        let mut page_info_vec: Vec<PageInfo> =
            match serde_json::from_str(page_info_vec_content.as_str()) {
                Ok(vec) => vec,
                Err(_) => Vec::new(),
            };
        ...
        for verb_url in verb_url_vec {
            ...
            let scraped_verb_content: String =
                match read_to_string(scraped_verb_vec_file_path.as_str()) {
                    Ok(content) => content,
                    Err(_) => {
            ...
            let mut scraped_verb_vec: Vec<String> =
                match serde_json::from_str(&scraped_verb_content) {
                    Ok(content) => content,
                    Err(_) => Vec::new(),
                };
            ...
            page_info.metadata.model = document
                ...
                .trim()
                .to_string();
            page_info.metadata.base = document
                ...
                .trim()
                .to_string();
            page_info.metadata.auxiliary = document
                .select(&auxiliary_selector)
                .map(|auxiliary| auxiliary.text().collect::<String>().trim().to_string())
                .collect::<Vec<String>>();
            page_info.metadata.forms = document
                .select(&forms_selector)
                .map(|form| form.text().collect::<String>().trim().to_string())
                .collect::<Vec<String>>();
            page_info.metadata.similar_verbs = document
                .select(&similar_verbs_selector)
                .map(|similar| similar.text().collect::<String>().trim().to_string())
                .collect::<Vec<String>>();
            page_info.metadata.other_verbs = document
                .select(&other_verbs_selector)
                .map(|other| other.text().collect::<String>().trim().to_string())
                .collect::<Vec<String>>();
            ...
            page_info.subjects = document
                .select(&subjects_selector)
                .map(|subject| subject.text().collect::<String>().trim().to_string())
                .collect::<Vec<String>>()
                .into_iter()
                .unique()
                .collect::<Vec<String>>();
            page_info.auxiliaries = document
                .select(&auxiliaries_selector)
                .map(|auxiliary| auxiliary.text().collect::<String>().trim().to_string())
                .collect::<Vec<String>>()
                .into_iter()
                .unique()
                .collect::<Vec<String>>();
            page_info.conjugates = document
                .select(&conjugates_selector)
                .map(|conjugate| conjugate.text().collect::<String>().trim().to_string())
                .collect::<Vec<String>>()
                .into_iter()
                .unique()
                .collect::<Vec<String>>();
            page_info.particles = document
                .select(&particles_selector)
                .map(|particle| particle.text().collect::<String>().trim().to_string())
                .collect::<Vec<String>>()
                .into_iter()
                .unique()
                .collect::<Vec<String>>();
            ...
            // Get tenses
            let tense_vec: Vec<String> = main_section
                .select(&tense_selector)
                .map(|tense| {
                    tense
                        .value()
                        .attr("mobile-title")
            ...
            let major_tense_vec = main_section
                .select(&major_tense_selector)
                .map(|major_tense| major_tense.text().collect::<String>().trim().to_string())
                .collect::<Vec<String>>()
                .into_iter()
                .unique()
                .collect::<Vec<String>>();
            ...
            for tense in &tense_vec {
                ...
                for major_tense in &major_tense_vec {
                    if tense.contains(major_tense) {
                        ...
                        if minor_tense.len() > 0 {
                            minor_tense_vec.push(minor_tense.clone())
                        };
            ...
            for mini_section in main_section.select(&mini_section) {
                ...
                for phrase_section in mini_section.select(&phrase_selector) {
                    let mut phrase: Vec<String> = phrase_section
                        .select(&i_selector)
                        .map(|phrase| {
                    ...
                    let mut subjects: Vec<String> = phrase_section
                        .select(&subjects_selector)
                        .map(|subject| subject.text().collect::<String>().trim().to_string())
                        .collect::<Vec<String>>();
                    ...
                    let mut auxiliaries: Vec<String> = phrase_section
                        .select(&auxiliaries_selector)
                        .map(|auxiliary| auxiliary.text().collect::<String>().trim().to_string())
                        .collect::<Vec<String>>();
                    ...
                    let conjugates_fused: Vec<String> = phrase_section
                        .select(&conjugates_fused_selector)
                        .map(|test| test.text().collect::<String>().trim().to_string())
                        .collect::<Vec<String>>();
                    let mut conjugates: Vec<String> = phrase_section
                        .select(&conjugates_selector)
                        .map(|conjugate| conjugate.text().collect::<String>().trim().to_string())
                        .collect::<Vec<String>>();
                    ...
                    let mut particles: Vec<String> = phrase_section
                        .select(&particles_selector)
                        .map(|particle| particle.text().collect::<String>().trim().to_string())
                        .collect::<Vec<String>>();
...
async fn extend_verb_page_info_vec_vec(
    language_vec: Vec<String>,
    mut verb_page_info_vec_vec: Vec<Vec<PageInfo>>,
    mut backoff: u64,
    mut error_429_backoff: u64,
) -> Vec<Vec<PageInfo>> {
    for verb_page_info_vec_index in 0..verb_page_info_vec_vec.len() {
        let page_info_vec_file_path: String = String::from("temp/json/conjugations/data/")
            + language_vec[verb_page_info_vec_index].as_str()
            + "_page_info.json";
        let scraped_verb_vec_file_path: String = String::from("temp/json/conjugations/data/")
            + language_vec[verb_page_info_vec_index].as_str()
            + "_scraped_verb_vec.json";
        ...
        for verb_page_info_index in 0..verb_page_info_vec_vec[verb_page_info_vec_index].len() {
            ...
            let mut verb_vec: Vec<String> = verb_page_info_vec_vec[verb_page_info_vec_index]
                [verb_page_info_index]
                .metadata
                .similar_verbs
                .clone();
            verb_vec.extend(
                verb_page_info_vec_vec[verb_page_info_vec_index][verb_page_info_index]
                    .metadata
                    .other_verbs
                    .clone(),
            );
            ...
            for verb in &verb_vec {
                ...
                let count: u64 = verb_page_info_vec_vec[verb_page_info_vec_index]
                    .last()
                    .unwrap()
                    .metadata
                    .rank
                ...
                while valid_response_bool == false {
                    ...
                    if valid_response_bool == false {
                        ...
                        // save new backoff to env
                            ...
                            error_429_backoff =
                                ((error_429_backoff + 1) as f64 * 1.2).round() as u64;
                            env::set_var("ERROR_429_BACKOFF", error_429_backoff.to_string());
                            save_env("ERROR_429_BACKOFF", &error_429_backoff.to_string()).unwrap();
                            // save new error_429_backoff to env
                        }
                ...
                let model_selector = scraper::Selector::parse("span#ch_lblModel>a").unwrap();
                let base_selector = scraper::Selector::parse("a#ch_lblVerb").unwrap();
                let auxiliary_selector =
                    scraper::Selector::parse("span#ch_lblAuxiliary>a").unwrap();
                let forms_selector = scraper::Selector::parse("span#ch_lblAutreForm>a").unwrap();
                let similar_verbs_selector =
                    scraper::Selector::parse("div.word-wrap-descr>a").unwrap();
                let other_verbs_selector =
                    scraper::Selector::parse("div.verb-others-list>a").unwrap();
                ...
                let auxiliaries_selector = scraper::Selector::parse("i.auxgraytxt").unwrap();
                let conjugates_selector = scraper::Selector::parse("i.verbtxt").unwrap();
                let conjugates_irr_selector =
                    scraper::Selector::parse("i.verbtxt-term-irr").unwrap();
                let particles_selector = scraper::Selector::parse("i.particletxt").unwrap();
                ...
                let main_section_selector = scraper::Selector::parse("div.word-wrap").unwrap();
                let tense_selector = scraper::Selector::parse("div[mobile-title]").unwrap();
                let major_tense_selector =
                    scraper::Selector::parse("div.word-wrap-title>h4").unwrap();
                ...
                for mini_section in main_section.select(&mini_section) {
                    ...
                    for phrase_section in mini_section.select(&phrase_selector) {
                        let mut phrase: Vec<String> = phrase_section
                            .select(&i_selector)
                            .map(
                                |phrase| match phrase.first_child().unwrap().has_children() {
                                    true => phrase.text().collect::<String>().trim().to_string(),
                                    false => {
                                        let parent_value = phrase.parent().unwrap().value();
                                        if let Node::Element(element) = parent_value {
                        ...
                        let mut auxiliaries: Vec<String> = phrase_section
                            .select(&auxiliaries_selector)
                            .map(|auxiliary| {
                                auxiliary.text().collect::<String>().trim().to_string()
                            })
                            .collect::<Vec<String>>();
                        auxiliaries = match auxiliaries.len() {
                            0 => Vec::new(),
                        ...
                        let mut conjugates: Vec<String> = phrase_section
                            .select(&conjugates_selector)
                            .map(|conjugate| {
                                conjugate.text().collect::<String>().trim().to_string()
                            })
                            .collect::<Vec<String>>();
                        conjugates = match conjugates_fused.len() {
                            0 => match conjugates.len() {
                ...
                save_data_to_file(
                    &verb_page_info_vec_vec[verb_page_info_vec_index],
                    &page_info_vec_file_path,
                );
                save_data_to_file(&scraped_verb_vec, &scraped_verb_vec_file_path);
            }
        }
...
fn extract_base_data_vec_vec(
    verb_page_info_vec_vec: &Vec<Vec<PageInfo>>,
    ...
    for (index, verb_page_info_vec) in verb_page_info_vec_vec.into_iter().enumerate() {
        for verb_page_info in verb_page_info_vec.into_iter() {
            ...
            let language: String = language_pk_map
                .get(&verb_page_info.metadata.language.clone())
                .unwrap()
                .to_string();
...
fn get_base_pk_map_vec(
    base_json_data_vec: Vec<JsonData>,
    language_vec: &Vec<String>,
) -> Vec<BTreeMap<String, i64>> {
    let mut base_pk_map_vec: Vec<BTreeMap<String, i64>> =
        language_vec.into_iter().map(|_| BTreeMap::new()).collect();
    for base_data in base_json_data_vec {
        if let Field::BaseField(BaseField {
            rank: _,
            base,
            language,
        }) = &base_data.fields
        {
            ...
            base_pk_map_vec[language_id.to_string().parse::<usize>().unwrap() - 1]
                .insert(base.to_owned(), base_data.pk);
        }
    }
...
fn extract_major_tense_data_vec_vec(
    verb_page_info_vec_vec: &Vec<Vec<PageInfo>>,
    language_pk_map: &BTreeMap<String, i64>,
) -> Vec<Vec<String>> {
    ...
    for (index, verb_page_info_vec) in verb_page_info_vec_vec.into_iter().enumerate() {
        ...
        for verb_page_info in verb_page_info_vec.into_iter() {
            ...
            for major_tense_data in major_tense_data_vec {
                if major_tense_vec.contains(&major_tense_data) {
                    continue;
                }
                major_tense_vec.push(major_tense_data.clone());

                let major_tense: Vec<String> =
                    Vec::from([language.clone(), major_tense_data.major]);
...
fn get_major_tense_pk_map_vec(
    major_tense_json_data_vec: Vec<JsonData>,
    language_vec: &Vec<String>,
) -> Vec<BTreeMap<String, i64>> {
    let mut major_tense_pk_map_vec: Vec<BTreeMap<String, i64>> =
        language_vec.into_iter().map(|_| BTreeMap::new()).collect();
    // let mut count_vec: Vec<i64> = language_vec.into_iter().map(|_| 0).collect();
    for major_tense_data in major_tense_json_data_vec {
        if let Field::MajorTenseField(MajorTenseField {
            language,
            major_tense,
        }) = &major_tense_data.fields
        {
            let language_id: i64 = language.parse::<i64>().unwrap();
            major_tense_pk_map_vec[language_id as usize - 1]
                .insert(major_tense.clone(), major_tense_data.pk);
...
fn extract_minor_tense_data_vec_vec(
    verb_page_info_vec_vec: &Vec<Vec<PageInfo>>,
    language_pk_map: &BTreeMap<String, i64>,
) -> Vec<Vec<String>> {
    ...
    for (index, verb_page_info_vec) in verb_page_info_vec_vec.into_iter().enumerate() {
        ...
        for verb_page_info in verb_page_info_vec.into_iter() {
            ...
            for minor_tense_data in minor_tense_data_vec {
                if minor_tense_vec.contains(&minor_tense_data) {
                    continue;
                }
                minor_tense_vec.push(minor_tense_data.clone());

                let minor_tense: Vec<String> =
                    Vec::from([language.clone(), minor_tense_data.minor]);
...
fn get_minor_tense_pk_map_vec(
    minor_tense_json_data_vec: Vec<JsonData>,
    language_vec: &Vec<String>,
) -> Vec<BTreeMap<String, i64>> {
    let mut minor_tense_pk_map_vec: Vec<BTreeMap<String, i64>> =
        language_vec.into_iter().map(|_| BTreeMap::new()).collect();
    for minor_tense_data in minor_tense_json_data_vec {
        if let Field::MinorTenseField(MinorTenseField {
            language,
            minor_tense,
        }) = &minor_tense_data.fields
        {
            let language_id: i64 = language.parse::<i64>().unwrap();
            minor_tense_pk_map_vec[language_id.to_string().parse::<usize>().unwrap() - 1]
                .insert(minor_tense.clone(), minor_tense_data.pk);
        }
    }
...
fn extract_tense_data_vec_vec(
    verb_page_info_vec_vec: &Vec<Vec<PageInfo>>,
    major_tense_pk_map: &Vec<BTreeMap<String, i64>>,
    minor_tense_pk_map: &Vec<BTreeMap<String, i64>>,
) -> Vec<Vec<String>> {
    ...
    for (language_index, verb_page_info_vec) in verb_page_info_vec_vec.into_iter().enumerate() {
        ...
        for verb_page_info in verb_page_info_vec.into_iter() {
            ...
            for (index, tense_data) in tense_data_vec.into_iter().enumerate() {
                if tense_vec.contains(&tense_data) {
                    continue;
                }
                tense_vec.push(tense_data.clone());

                let rank: String = (index + 1).to_string();
                let major_tense: String = major_tense_pk_map[language_index]
                    .get(&tense_data.major)
                    .unwrap()
                    .to_string();
                let minor_tense: String = minor_tense_pk_map[language_index]
                    .get(&tense_data.minor)
                    .unwrap()
                    .to_string();
...
fn get_tense_pk_map_vec(
    tense_json_data_vec: Vec<JsonData>,
    language_vec: &Vec<String>,
    major_tense_pk_map_vec: &Vec<BTreeMap<String, i64>>,
) -> Vec<BTreeMap<String, i64>> {
    let mut tense_pk_map_vec: Vec<BTreeMap<String, i64>> =
        language_vec.into_iter().map(|_| BTreeMap::new()).collect();
    for tense_data in tense_json_data_vec {
        if let Field::TenseField(TenseField { rank: _, tense }) = &tense_data.fields {
            ...
            tense_pk_map_vec[language_id].insert(
                tense.major.clone() + "|" + tense.minor.as_str(),
                tense_data.pk,
            );
        }
    }
...
fn extract_particle_data_vec_vec(
    verb_page_info_vec_vec: &Vec<Vec<PageInfo>>,
    language_pk_map: &BTreeMap<String, i64>,
) -> Vec<Vec<String>> {
    ...
    for verb_page_info_vec in verb_page_info_vec_vec {
        let language: String = language_pk_map
            .get(&verb_page_info_vec[0].metadata.language.clone())
            .unwrap()
            .to_string();
        ...
        for verb_page_info in verb_page_info_vec {
            ...
            for particle in particles {
                if particle_vec.contains(&particle) {
                    continue;
                }
...
fn get_particle_pk_map_vec(
    subject_json_data_vec: Vec<JsonData>,
    language_vec: &Vec<String>,
) -> Vec<BTreeMap<String, i64>> {
    let mut particle_pk_map_vec: Vec<BTreeMap<String, i64>> =
        language_vec.into_iter().map(|_| BTreeMap::new()).collect();
    for particle_data in subject_json_data_vec {
        if let Field::ParticleField(ParticleField { particle, language }) = &particle_data.fields {
            let language_id: i64 = language.parse::<i64>().unwrap();
            particle_pk_map_vec[language_id.to_string().parse::<usize>().unwrap() - 1]
                .insert(particle.to_owned(), particle_data.pk);
        }
    }
...
fn extract_subject_data_vec_vec(
    verb_page_info_vec_vec: &Vec<Vec<PageInfo>>,
    language_pk_map: &BTreeMap<String, i64>,
) -> Vec<Vec<String>> {
    ...
    for verb_page_info_vec in verb_page_info_vec_vec {
        ...
        for verb_page_info in verb_page_info_vec.into_iter() {
            ...
            for (index2, subject) in subjects.into_iter().enumerate() {
                if subject_vec.contains(&subject) {
                    continue;
                }
...
fn get_subject_pk_map_vec(
    subject_json_data_vec: Vec<JsonData>,
    language_vec: &Vec<String>,
) -> Vec<BTreeMap<String, i64>> {
    let mut subject_pk_map_vec: Vec<BTreeMap<String, i64>> =
        language_vec.into_iter().map(|_| BTreeMap::new()).collect();
    for subject_data in subject_json_data_vec {
        if let Field::SubjectField(SubjectField {
            rank: _,
            subject,
            language,
        }) = &subject_data.fields
        {
            let language_id: i64 = language.parse::<i64>().unwrap();
            subject_pk_map_vec[language_id.to_string().parse::<usize>().unwrap() - 1]
                .insert(subject.to_owned(), subject_data.pk);
        }
    }
...
fn extract_auxiliary_data_vec_vec(
    verb_page_info_vec_vec: &Vec<Vec<PageInfo>>,
    language_pk_map: &BTreeMap<String, i64>,
) -> Vec<Vec<String>> {
    ...
    for verb_page_info_vec in verb_page_info_vec_vec {
        ...
        for verb_page_info in verb_page_info_vec.into_iter() {
            ...
            for auxiliary in auxiliaries {
                if auxiliary_vec.contains(&auxiliary) {
                    continue;
                }
...
fn get_auxiliary_pk_map_vec(
    auxiliary_json_data_vec: Vec<JsonData>,
    language_vec: &Vec<String>,
) -> Vec<BTreeMap<String, i64>> {
    let mut auxiliary_pk_map_vec: Vec<BTreeMap<String, i64>> =
        language_vec.into_iter().map(|_| BTreeMap::new()).collect();
    for auxiliary_data in auxiliary_json_data_vec {
        if let Field::AuxiliaryField(AuxiliaryField {
            auxiliary,
            language,
        }) = &auxiliary_data.fields
        {
            let language_id: i64 = language.parse::<i64>().unwrap();
            auxiliary_pk_map_vec[language_id.to_string().parse::<usize>().unwrap() - 1]
                .insert(auxiliary.to_owned(), auxiliary_data.pk);
        }
    }
...
fn extract_conjugate_data_vec_vec(
    verb_page_info_vec_vec: &Vec<Vec<PageInfo>>,
    base_pk_map_vec: &Vec<BTreeMap<String, i64>>,
    model_pk_map_vec: &Vec<BTreeMap<String, i64>>,
) -> Vec<Vec<String>> {
    let mut conjugate_data_vec_vec: Vec<Vec<String>> = Vec::new();

    for (index, verb_page_info_vec) in verb_page_info_vec_vec.into_iter().enumerate() {
        for verb_page_info in verb_page_info_vec.into_iter() {
            let base: String = base_pk_map_vec[index]
                .get(&verb_page_info.metadata.base)
                .unwrap()
                .to_string();
            // println!("base: {:?}, base_id: {:?}", verb_page_info.metadata.base.clone(), base);
            let model: String =
                match model_pk_map_vec[index].get(&verb_page_info.metadata.model.clone()) {
                    None => String::from("-"),
            ...
            for conjugate in conjugates.clone() {
                let conjugate_data_vec: Vec<String> =
                    Vec::from([0.to_string(), base.clone(), model.clone(), conjugate]);
...
fn get_conjugate_pk_map_vec(
    conjugate_json_data_vec: Vec<JsonData>,
    base_json_data_vec: Vec<JsonData>,
    language_vec: &Vec<String>,
) -> Vec<BTreeMap<String, i64>> {
    let mut conjugate_pk_map_vec: Vec<BTreeMap<String, i64>> =
        language_vec.into_iter().map(|_| BTreeMap::new()).collect();
    for conjugate_data in conjugate_json_data_vec {
        if let Field::ConjugateField(ConjugateField {
            rank: _,
            base,
            model: _,
            conjugate,
        }) = &conjugate_data.fields
        {
            ...
            if let Field::BaseField(BaseField {
                ...
                language,
                base: _,
            }) = base_json_data_vec[base_pk.to_string().parse::<usize>().unwrap()]
                .clone()
                .fields
            {
            ...
            conjugate_pk_map_vec[language_id.to_string().parse::<usize>().unwrap() - 1]
                .insert(conjugate.to_owned(), conjugate_data.pk);
        }
    }
...
fn extract_conjugation_data_vec_vec(
    verb_page_info_vec_vec: &Vec<Vec<PageInfo>>,
    major_tense_pk_map: &Vec<BTreeMap<String, i64>>,
    minor_tense_pk_map: &Vec<BTreeMap<String, i64>>,
    tense_pk_map_vec: &Vec<BTreeMap<String, i64>>,
    particle_pk_map_vec: &Vec<BTreeMap<String, i64>>,
    subject_pk_map_vec: &Vec<BTreeMap<String, i64>>,
    auxiliary_pk_map_vec: &Vec<BTreeMap<String, i64>>,
    conjugate_pk_map_vec: &Vec<BTreeMap<String, i64>>,
) -> Vec<Vec<String>> {
    ...
    for (index, verb_page_info_vec) in verb_page_info_vec_vec.into_iter().enumerate() {
        for verb_page_info in verb_page_info_vec.into_iter() {
            ...
            for (index2, phrase_vec) in phrases.into_iter().enumerate() {
                let major_tense: String = major_tense_pk_map[index]
                    .get(&verb_page_info.tenses[index2].major)
                    .unwrap()
                    .to_string();
                let minor_tense: String = minor_tense_pk_map[index]
                    .get(&verb_page_info.tenses[index2].minor)
                    .unwrap()
                    .to_string();
                let tense = tense_pk_map_vec[index]
                    .get(&(major_tense + "|" + &minor_tense))
                    .unwrap()
                    .to_string();
                ...
                for phrase in phrase_vec {
                    ...
                    for particle in phrase.clone().particles {
                        let particle_pk = particle_pk_map_vec[index]
                            .get(&particle)
                            .unwrap()
                            .to_string();
                    ...
                    for subject in phrase.clone().subjects {
                        let subject_pk =
                            subject_pk_map_vec[index].get(&subject).unwrap().to_string();
                    ...
                    for auxiliary in phrase.clone().auxiliaries {
                        let auxiliary_pk = auxiliary_pk_map_vec[index]
                            .get(&auxiliary)
                            .unwrap()
                            .to_string();
                    ...
                    for conjugate in phrase.clone().conjugates {
                        ...
                        println!("index: {:?}", index);
                        let conjugate_pk = conjugate_pk_map_vec[index]
                            .get(&conjugate)
                            .unwrap()
                            .to_string();
                    ...
                    let conjugation_data_vec: Vec<String> = Vec::from([
                        rank.clone(),
                        tense.clone(),
                        particle,
                        subject,
